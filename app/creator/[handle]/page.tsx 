...
export default function CreatorProfilePage() {
  ...
  // Log a profile view once we know whose profile this is (skips self-views on the backend)
  useEffect(() => {
    if (profile?.id) {
  ...
  if (notFound || !profile) {
    return (
      <div className="max-w-md mx-auto pb-24 px-4 pt-6 text-center">
        ...
        {debugError && (
          <p className="text-[11px] text-red-500 mt-4 max-w-xs mx-auto break-words">
            Debug: {debugError}
          </p>
        )}
      </div>
  ...
  return (
    <div className="max-w-md mx-auto pb-24 px-4 pt-6">
      ...
      <div className="flex items-center gap-6 mt-4">
        ...
        <Link href={`/followers/${profile.id}?mode=followers`} className="text-center">
          <p className="text-base font-bold text-stone-900">{profile.followers_count}</p>
          <p className="text-[11px] text-stone-500">Followers</p>
        </Link>
        <Link href={`/followers/${profile.id}?mode=following`} className="text-center">
