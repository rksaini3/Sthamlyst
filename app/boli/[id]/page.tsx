...
export default function BoliDetailPage() {
  ...
  // Debug jaankari — agar auction na mile, to yahan exact wajah
  // store hoti hai aur screen ke neeche dikhayi jaati hai (dev ke liye).
  ...
  async function loadAuction() {
    if (!auctionId) {
      console.error('loadAuction: auctionId khud hi undefined/empty hai — URL check karo')
      setLoadDebugInfo('URL mein auction ID nahi mili (params.id empty tha).')
      return
    }
    ...
    const { data, error: fetchError } = await supabase
      .from('auctions')
      .select(
        'id, base_price, current_highest_bid, end_time, status, seller_id, products ( title, image_url, maker_name, voice_note_url )'
      )
      .eq('id', auctionId)
      .maybeSingle() // .single() ki jagah — taaki "0 rows" par bhi throw na ho, sirf null aaye

    if (fetchError) {
      // Yahi asli jagah hai jahan pehle chup-chaap return ho raha tha.
      // Ab exact Supabase error console mein dikhega.
      console.error('loadAuction Supabase error:', {
      ...
      setLoadDebugInfo(
        `Supabase error: ${fetchError.message}${fetchError.hint ? ' | Hint: ' + fetchError.hint : ''}`
      )
      return
    }

    if (!data) {
      console.warn('loadAuction: query safal rahi lekin is auctionId ki koi row nahi mili:', auctionId)
      setLoadDebugInfo(
        `ID "${auctionId}" ki koi auction row nahi mili. Ya to ID galat hai, ya row delete ho chuki hai.`
      )
      return
    }

    // products join null bhi aa sakta hai agar products table ki RLS
    // policy is specific product ko block kar rahi ho.
    const productData = data.products as unknown as AuctionDetail['product'] | null
    if (!productData) {
      console.warn('loadAuction: auction mili, lekin joined product null aaya — products table ki RLS policy check karo:', auctionId)
      setLoadDebugInfo(
        'Auction row mili, lekin uska product data nahi aaya — products table ki RLS policy check karein.'
      )
      return
    }
  ...
  async function loadBids() {
    ...
    if (bidsError) {
      console.error('loadBids Supabase error:', bidsError.message)
      return
    }
    if (!data) return
    ...
    if (profilesError) {
      console.error('loadBids profiles fetch error:', profilesError.message)
    }
  ...
  async function placeBid() {
    ...
    try {
      ...
      if (audioBlob) {
        ...
        if (uploadErr) {
          console.error('Bid voice upload error:', uploadErr.message)
        } else {
      ...
      const { error: updateError } = await supabase
        ...
        .eq('id', auction.id)
      if (updateError) {
        console.error('Auction highest-bid update error:', updateError.message)
      }
  ...
  if (!auction) {
    return (
      <div className="max-w-md mx-auto min-h-dvh flex flex-col items-center justify-center px-6 text-center">
        ...
        <Link href="/boli" className="mt-4 text-sm font-semibold text-clay underline">
          Boli Board pe wapas jaayein
        </Link>
        {/* Dev-debug line — production mein hata sakte ho, abhi diagnosis ke liye rakha hai */}
        {loadDebugInfo && (
          <p className="mt-6 text-[11px] text-stone-400 bg-stone-50 dark:bg-stone-800 rounded-lg px-3 py-2 max-w-xs break-words">
            Debug: {loadDebugInfo}
          </p>
        )}
      </div>
  ...
  return (
    <div className="max-w-md mx-auto pb-24">
      ...
      <div className="px-4 pt-4">
        ...
        <div className="flex items-center justify-between mt-3 bg-stone-50 dark:bg-stone-800 rounded-2xl p-4">
          ...
          <span
            className={`text-xs font-bold px-3 py-1.5 rounded-full ${
              isEnded ? 'bg-stone-700 text-white' : 'bg-clay/10 text-clay'
            }`}
          >
        ...
        <h2 className="text-sm font-bold text-stone-800 dark:text-stone-100 mt-6 mb-2">
          सारी बोलियां ({bids.length})
        </h2>
            ...
            bids.map((bid, i) => (
              <div
                ...
                className={`flex items-center gap-2.5 rounded-xl px-3 py-2.5 ${
                  i === 0 ? 'bg-mehendi-light' : 'bg-stone-50 dark:bg-stone-800'
                }`}
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {bid.voice_note_url && (
                    <audio src={bid.voice_note_url} controls className="h-6 mt-0.5 max-w-full" />
                  )}
                </div>
                <p className={`text-sm font-bold flex-shrink-0 ${i === 0 ? 'text-mehendi' : 'text-stone-600'}`}>
                  ₹{bid.amount}
                </p>
              </div>
            ))
